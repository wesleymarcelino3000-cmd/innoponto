...
import { SUPABASE_URL, SUPABASE_KEY, LOCAL_EMPRESA, LIMITE_METROS } from './config.js'
...
function irAdmin(){ const u=getUser(); if(u?.role==='admin') location.href='admin.html'; else alert('Somente admin pode acessar.') }
function voltarPainel(){ location.href='painel.html' }
...
window.registrarPonto = async function(tipo){
  ...
  navigator.geolocation.getCurrentPosition(async pos=>{
    ...
    showMsg('status', `Ponto registrado dentro da área permitida (${Math.round(distancia)}m).`, false, 'success')
...
async function carregarMeusRegistros(){
  ...
  if(!tbody) return
  const user = requireLogin()
  const { data } = await supabase.from('registros').select('*').eq('usuario', user.usuario).order('data', { ascending: false }).limit(80)
  const regs = data || []
  tbody.innerHTML = regs.length ? regs.map(r => `<tr><td>${formatDateTime(r.data)}</td><td>${r.tipo}</td><td>${r.observacao || '-'}</td><td>${r.latitude || '-'}</td><td>${r.longitude || '-'}</td></tr>`).join('') : '<tr><td colspan="5">Nenhum registro.</td></tr>'
}
...
window.salvarConfiguracoes = async function(){
  ...
  let result
  if(existing){
    result = await supabase.from('configuracoes').update({ jornada_horas: jornada, tolerancia_min: tolerancia }).eq('id', existing.id)
  } else {
    result = await supabase.from('configuracoes').insert({ jornada_horas: jornada, tolerancia_min: tolerancia })
  }
...
window.fecharMes = async function(){
  ...
  const { error } = await supabase.from('fechamentos').insert({ mes_ref: mes, fechado_por: user.usuario, fechado_em: new Date().toISOString() })
...
window.criarUsuario = async function(){
  ...
  showMsg('userMsg', error ? 'Erro ao criar usuário. Verifique se ele já existe.' : 'Usuário criado com sucesso.', false, error ? 'danger' : 'success')
  if(!error){
...
window.exportarExcel = async function(){
  ...
  for (const r of rows) {
    ...
    if (!agrupado[chave]) {
      agrupado[chave] = { usuario: r.usuario, data: dataBr, entrada:'', saidaAlmoco:'', voltaAlmoco:'', saida:'', entradaISO:null, saidaAlmocoISO:null, voltaAlmocoISO:null, saidaISO:null }
    }
  ...
  XLSX.writeFile(wb, `Ponto_InnoLife_${String(agora.getMonth()+1).padStart(2,'0')}_${agora.getFullYear()}.xlsx`)
}
