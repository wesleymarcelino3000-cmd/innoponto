...
window.carregarRegistrosAdmin = async function(){
  ...
  const regs = data || []
  tbody.innerHTML = regs.length ? regs.map(r => `<tr><td><input id="r_usuario_${r.id}" value="${r.usuario ?? ''}"></td><td><input id="r_data_${r.id}" value="${r.data ?? ''}"></td><td><input id="r_tipo_${r.id}" value="${r.tipo ?? ''}"></td><td><input id="r_obs_${r.id}" value="${r.observacao ?? ''}"></td><td><div class="toolbar"><button onclick="salvarRegistro('${r.id}')" style="width:auto">Salvar</button><button class="danger" onclick="excluirRegistro('${r.id}')" style="width:auto">Excluir</button></div></td></tr>`).join('') : '<tr><td colspan="5">Nenhum registro.</td></tr>'
}
